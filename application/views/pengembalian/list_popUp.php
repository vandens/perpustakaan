...
<h1><?php echo "Daftar Pengembalian Buku"; ?></h1>

<div id='isi'>
<form name="form1" method="post" action="<?=base_url()?>/pengembalian">
<?php echo $notif;?>
<table border='0px' width='100%' class='TableContent'>
	<tr class='TableHeader'>
		...
		<td width="125">Tgl Wjb Kembali</td>
	...
	if ($data->num_rows() > 0){
		foreach($data->result_array() as $dp){
			...
			$kbl = $dp['tgl_wjb_kembali'];
			$skg = date('Y-m-d');
			if($kbl < $skg)

				$flag = '<img src="'.base_url('media/img/25.png').'" title="Wajib Kembali">';
			else

				$flag = '<img src="'.base_url('media/img/22.png').'" title="Dalam Peminjaman">';
	?>	
	...
	<div style="float:right;">
			<input type='submit' name='kembali' value='Kembalikan' <?=$btn;?>>
			<!--
